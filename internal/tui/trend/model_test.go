...
// TestTrendModel_GetVortexCounts_MixedSignals verifies GetVortexCounts correctly
// counts LONG, SHORT, and HOLD VORTEX signals.
func TestTrendModel_GetVortexCounts_MixedSignals(t *testing.T) {
	model := newTestModel(t, &mockEngine{}, "AAPL", "MSFT", "GOOGL", "NVDA", "AMZN")

	vortexScores := []int{1, 1, -1, 0, -1}

	for i, score := range vortexScores {
	...
	long, short, hold := model.GetVortexCounts()
...
// TestTrendModel_GetVortexCounts_IgnoresLoadingRows verifies GetVortexCounts
// ignores rows that are still loading or in error state.
func TestTrendModel_GetVortexCounts_IgnoresLoadingRows(t *testing.T) {
	model := newTestModel(t, &mockEngine{}, "AAPL", "MSFT", "GOOGL")
...
// TestTrendModel_GetVortexCounts_EmptyRows verifies GetVortexCounts returns
// all zeros when there are no loaded rows.
func TestTrendModel_GetVortexCounts_EmptyRows(t *testing.T) {
	model := newTestModel(t, &mockEngine{})
...
// TestTrendModel_GetVortexCounts_CachedRows verifies GetVortexCounts
// includes cached rows in count.
func TestTrendModel_GetVortexCounts_CachedRows(t *testing.T) {
	model := newTestModel(t, &mockEngine{}, "AAPL")

	// Set row to loaded
	result := &trenddomain.Result{Symbol: "AAPL", Signal: trenddomain.Bullish, VortexScore: 1}
	updatedM, _ := model.Update(TrendDataMsg{Symbol: "AAPL", Result: result})
	model = asModel(t, updatedM)
